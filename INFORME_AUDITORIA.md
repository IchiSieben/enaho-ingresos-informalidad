...
| **AC-5** | **Cifra externa del INEI presentada con una etiqueta que el lector mapea a una categoría propia.** El README, la ficha técnica de la app y dos documentos afirmaban que «el gradiente por tamaño de empresa del modelo replica el patrón oficial (**88,6 %** de informalidad en microempresas vs 15,6 % en grandes)». Ver el desarrollo completo abajo. | `src/08_ablacion_clasificador.py:102` (origen, escrito a mano) | **Resuelto.** App: el gradiente sale de `tasas_observadas` (artefacto). README (ES/EN) y `docs/` citan el 88,6 % como cifra del INEI con su tramo 1-10 explícito. Verificado el 24/09/2026 por búsqueda de texto: ninguna aparición restante lo presenta como dato propio. |
| **AC-6** | **Afirmación de literatura sin fuente y con el rango equivocado.** La ficha técnica dice que un R² de «0,4–0,5 es techo habitual con encuestas de hogares». No hay referencia que lo respalde en el repositorio, y el rango que la literatura de ecuaciones de ingresos reporta (Lemieux 2006) es más bien **0,2–0,35**. Además mezcla dos cosas distintas: el R² de 0,42 es del modelo E9 en soles, mientras que el rango de la literatura se refiere a ecuaciones tipo Mincer en logaritmo (aquí, E3: R² 0,27). | `app/streamlit_app.py:999` (antes de la v1.1) | **Resuelto.** La afirmación salió de la app. El texto canónico vive en `R2_MINCER_CANONICO` / `_EN` de `app/referencias.py` (Mincer 1974, tabla 5.1, R² 0,285; Card 1999, tabla 1, 0,247–0,328): separa el R² en soles de E9 del R² en logaritmo de E3 y aclara que Lemieux y Heckman et al. no reportan R². Verificado el 24/09/2026. |
| **AC-4** | **`n_jobs=20` en una máquina de 6 núcleos reales.** No es solo ineficiencia: durante la re-optimización, `RandomForestRegressor(n_jobs=20)` anidado dentro de `GridSearchCV(n_jobs=20)` mató los procesos worker de `loky` y tumbó una corrida de 70 minutos. | `reports/torneo_rejilla_ampliada.log`; núcleos confirmados con `Get-CimInstance Win32_Processor` | **Resuelto (24/09/2026).** `src/comun.py` define `N_JOBS = min(8, os.cpu_count())` y los scripts 04 y 06–09 lo importan de ahí; los estimadores anidados siguen con `n_jobs=1`. |
| **AC-7** | **`inei_informal` citaba una publicación que no mide el año que se le atribuye.** La app y el README le atribuían las cifras 2025 (70,2 % nacional, 64,5 % urbano, 94,8 % rural, 88,6 %/44,0 %/15,6 % por tamaño de empresa) a la Cuenta Satélite de la Economía Informal, que mide con EPEN «al 2024» y reporta 70,9 % nacional (no 70,2 %), con tramos de empresa distintos. Resuelve también NV-7 (la cifra oficial de contraste no tenía referencia verificable). | `docs/fase2/eje_7.md` (origen, documentación) | **Resuelto.** `inei_informal` en `app/referencias.py` ahora cita el Informe Técnico INEI «Comportamiento de los Indicadores del Mercado Laboral», IV trimestre 2025 (Cuadros 1.20 y 1.22, Gráfico 1.15, pp. 26-28), que sí mide 2025. Verificado el 26/09/2026: cuadro/gráfico y páginas cruzados contra `docs/fase2/eje_7.md`. |
| **AC-8** | **Se le atribuía a Heckman, Lochner y Todd (2006) un enunciado que el paper no hace.** La app decía que la medida de experiencia potencial (edad − escolaridad − 6) «sobreestima la experiencia efectiva» en trabajadores de baja educación, citando ese enunciado a Heckman et al. (2006). Búsqueda exhaustiva en el texto completo del paper: la fórmula sí está, la afirmación sobre sobreestimación no aparece. | `docs/MATRIZ_AFIRMACIONES.md`, fila 6 (origen, documentación) | **Resuelto.** `app/streamlit_app.py` ahora cita a Heckman et al. (2006) solo por la fórmula y marca la lectura sobre la sobreestimación como «lectura nuestra, no un resultado publicado», en ES y EN. |
...
**Estado (24/09/2026).** La validación externa quedó restaurada con el tramo
explícito del INEI en README y `docs/`. Sigue abierta la referencia
publicada de esas cifras del INEI: es NV-7.
